...
# 实测：100/101=跑步, 200=骑行, 203=砾石路骑行(2026-09-16 实测抓取), 104=徒步,
#       103/400/401/402/701=训练, 900=健走(步行), 9807=摩托骑行, 1000/9900=羽毛球(无FIT)
# 注：203(砾石路骑行) 按用户约定归入 moto —— 摩托模式不能自动暂停 GPS，改用砾石模式记录摩托出行。
COROS_TYPE_MAP = {
    100: "run", 101: "run",
    200: "ride",
    203: "moto",           # 砾石路骑行 → 摩托（标题改写见 _remap_moto）
    104: "hike",
